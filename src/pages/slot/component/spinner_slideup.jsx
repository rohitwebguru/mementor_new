  ...
  useEffect(() => {
    ...
    const slideUpContent = async (slideDuration, delayBetweenDoors) => {
      for (let i = 0; i < doors.length; i++) {
        ...
        boxes.style.transform = `translateY(0)`; // Reset transform
        boxes.style.opacity = 1; // Hide the content
        // Trigger reflow
        boxes.offsetHeight; // This line forces a reflow, which is necessary to apply the next transition smoothly
        boxes.style.transition = `transform ${slideDuration / 1000}s cubic-bezier(0.25, 0.46, 0.45, 0.94)`; // Re-enable transition
        boxes.style.transform = `translateY(-${doorHeight}px)`; // Slide content up
        await new Promise((resolve) => setTimeout(resolve, delayBetweenDoors)); // Wait for delay between doors
      }
      // Add a delay before resolving the promise to ensure the last door's content slides up completely
